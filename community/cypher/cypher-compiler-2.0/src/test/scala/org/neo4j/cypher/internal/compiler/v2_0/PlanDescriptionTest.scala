...
class PlanDescriptionTest extends Assertions {
  ...
  def should_generate_nested_java_description() {
    ...
    assert("linus" === result.getChildren.get(0).getName)
    assert("pauline" === result.getChildren.get(1).getName)
    assert("fritz" === result.getChildren.get(1).getChildren.get(0).getName)
  }
